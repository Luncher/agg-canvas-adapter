...
public:
  ...
  void set_text_align(const char* text_align);
  void set_text_baseline(const char* base_line);
  void fill_text(const char* text, double x, double y, double max_width);
  void measure_text(const char* text);
...
void Canvas::stroke()
{
  ...
  agg::conv_transform<agg::conv_stroke<agg::conv_curve<agg::path_storage> > > trans(stroke, m_mtx);
...
{
  m_mtx *= agg::trans_affine_rotation(angle * agg::pi / 180.0);
}
...
void Canvas::set_font(const char* font)
{
  m_text.font(font);
}

void Canvas::set_font_size(double size)
{
  m_text.size(size, size);
}

void Canvas::set_text_align(const char* text_align)
{
  //TODO
}

void Canvas::set_text_baseline(const char* base_line)
{
  //TODO
}

void Canvas::fill_text(const char* text, double x, double y, double max_width)
{
  //TODO
}

void Canvas::measure_text(const char* text)
{
  //TODO
}
...
public:
  ...
  {
    context->save();
    context->stroke_rect(450, 5, 25, 25);
    context->scale(1.1, 1.1);
    context->stroke_rect(450, 55, 25, 25);
    context->fill_rect(150,20,100,50);    
    context->rotate(60);
    context->fill_rect(150,20,100,50);
    context->restore();
    context->save();
    context->fill_rect(400,400,100,50);
    context->translate(70,70);
    context->fill_rect(400,400,100,50);
    context->restore();
    context->fill_rect(410,410,100,50);

    context->save();
    context->set_fill_color(1, 0.3, 0, 1);
    context->fill_rect(0, 0, 250, 100);

    context->transform(1, 0.5, -0.5, 1, 30, 10);
    printf("sx: %f, sy: %f, shx: %f, shy: %f, tx: %f, ty: %f\n", 
      context->m_mtx.sx, context->m_mtx.sy, context->m_mtx.shx, context->m_mtx.shy,
      context->m_mtx.tx, context->m_mtx.ty);
    context->set_fill_color(0.5, 0.3, 1, 1);
    context->fill_rect(0, 0, 250, 100);
    ...
    printf("sx: %f, sy: %f, shx: %f, shy: %f, tx: %f, ty: %f\n", 
      context->m_mtx.sx, context->m_mtx.sy, context->m_mtx.shx, context->m_mtx.shy,
      context->m_mtx.tx, context->m_mtx.ty);
    context->set_fill_color(0.9, 0.4, 1, 1);    
    context->fill_rect(0, 0, 250, 100);
